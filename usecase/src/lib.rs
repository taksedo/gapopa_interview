...
pub fn check_site_health_use_case(
    args: Vec<String>,
    ...
    let request_interval_millis =
        request.interval_duration_millisec.to_millis();

    match error_code {
        x if x.is_empty() => loop {
...
impl From<RequestError> for CheckHealthError {
    fn from(value: RequestError) -> Self {
        match value {
            ...
            RequestError::IntervalValueError => {
                CheckHealthError::IntervalValueError
            }
        }
    }
}
...
mod tests {
    ...
    fn site_name_error() {
        let args = vec![
            "".to_string(),
            "1".to_string(),
            "some_random_string".to_string(),
        ];
        let result = check_site_health_use_case(args);
        assert!(result.is_err());
        assert_eq!(
            result.unwrap_err().to_string(),
            "URL parsing error".to_string()
        );
    }
    ...
    fn number_of_args_error() {
        ...
        assert_eq!(
            result.unwrap_err().to_string(),
            "Number of arguments is wrong".to_string()
        );
    }
    ...
    fn interval_duration_parsing_error() {
        ...
        assert_eq!(
            result.unwrap_err().to_string(),
            "Wrong interval duration argument format".to_string()
        );
    }
}
